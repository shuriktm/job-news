                ...
                <tr>
                    ...
                    <td class="hstack gap-1">
                        @if (!$post->category->trashed())
                            <a class="btn btn-link text-primary p-0" href="{{ route('home.post', [$post->category, $post]) }}" title="{{ __('Public') }}" target="_blank">
                    ...
                    <td>
                        <form id="delete-{{ $post->id }}-form" method="POST" action="{{ route('posts.destroy', $post) }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-link text-danger p-0" title="{{ __('Delete') }}"
                                    onclick="event.preventDefault(); if (window.confirm('{{ __('Are you sure? Delete the item?') }}')) document.getElementById('delete-{{ $post->id }}-form').submit();">
