<form class="hstack gap-3" id="filter-form" action="{{ url()->full() }}" method="GET">
    <div class="input-group flex-nowrap">
        <select id="filter-category" name="category" class="form-select py-1 @if(request()->input('category')) bg-warning bg-opacity-10 @endif"
                onchange="event.preventDefault(); document.getElementById('filter-form').submit();">
            <option value="">{{ __('Category...') }}</option>
            @foreach ($categories as $key => $value)
                <option value="{{ $key }}" @selected($key == request()->input('category'))>{{ $value }}</option>
            @endforeach
        </select>
        <button class="btn btn-outline-secondary px-2 py-1 bg-light" type="button"
                onclick="event.preventDefault(); document.getElementById('filter-category').value = ''; document.getElementById('filter-form').submit();">
