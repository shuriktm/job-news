            ...
            @foreach ($categories as $index => $category)
                <tr>
                    ...
                    <td class="hstack gap-1">
                        <a class="btn btn-link text-primary p-0" href="{{ route('home.category', $category) }}" title="{{ __('Public') }}" target="_blank">
                    ...
                    <td>{{ $category->slug }}</td>
                    <td>
                        <form id="delete-{{ $category->id }}-form" method="POST" action="{{ route('categories.destroy', $category) }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-link text-danger p-0" title="{{ __('Delete') }}"
                                    onclick="event.preventDefault(); if (window.confirm('{{ __('Are you sure? Delete the item?') }}')) document.getElementById('delete-{{ $category->id }}-form').submit();">
